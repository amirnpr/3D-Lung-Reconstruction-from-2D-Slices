...
dir_name = uigetdir('"C:\Users\Lenovo\Dropbox\My PC (DESKTOP-BHVANOA)\Desktop\data_1'); 
files=dir(dir_name);
...
load('C:\Users\Lenovo\Dropbox\My PC (DESKTOP-BHVANOA)\Desktop\New folder\R_Data1','inputSlices');
T=inputSlices;
T = im2single(T);
XY = T(:,:,91); % center slice in xy (182/2)
...
% Seed Mask
mask = false(size(T));
mask(:,:,91) = maskedImageXY;
mask(256,:,:) = mask(256,:,:)|reshape(maskedImageXZ,[1,512,182]);
T = histeq(T);
